...
- **Fishing 船与露娜是整张贴图**（`assets/fishing/F_Luna.png` 300×300，船底龙骨对齐水面线，锚点 `LUNA_KEEL`），并预留 **`luna_view`** 接口：未来 tscn 动画场景 add_child 挂上去即接管绘制（`_process` 每帧自动对齐 `_luna_anchor() + _juice.world_offset()`，纯视觉节点，`_draw_boat()` 检测到就跳过贴图）。
