...
- 复古像素 + Y2K 风格；**只用 `shared/palette.gd` 的 24 色**（原 20 色＋2026-09 企划指定的 MOON_LIGHT 与排行榜主色三色 ACCENT_*），不新增中间色（唯一允许的变体是对既有色套 alpha）
...
| 标题（Title） | `launcher/launcher.gd` | 一级标题（`assets/title/Title_ChooseGames.png` 全屏）是管理员选择画面：↑↓ 循环选游戏（MAZE/FISHING/CATCH）、B 开清除选单、A 确认进二级标题。**二级标题背景是各款的全屏影片（`assets/title/TItleVideo/Title_*.ogv`，等比例缩放填满 480×270，素材未进场退回标题图 `Title_*.jpg`），没有按钮，底部一行闪烁的 PRESS ANY BUTTON TO START 提示（亮灭二值闪烁；起名 overlay 不画；待机影片功能已于 2026-09 移除）**：按**任意键**→ 起名开局（A/B 例外：**A＋B 同时按住 3 秒** → 管理员密码界面）；二级标题是固定场所，不响应 1/2/3 与 ESC，**唯一回一级的路径是管理员密码界面**（方向指令密码，见「管理员弹窗」行） |
...
| 排行榜（Leaderboard） | `shared/leaderboard_record.gd` / `leaderboard_manager.gd` / `date_utils.gd` + `ui/game_over.gd`（局终动画）+ `ui/leaderboard_panel.gd`（面板） | 本地存储 `user://leaderboard.json`（version:1，每款最多 1000 条，按 game_id 分榜）。三款共用同一套。游戏只发 `round_finished(score, duration, game_over)` 信号，不知道排行榜存在；组装与提交在 launcher，局终播 **Game Over 动画**（**背景保留游戏场景**——launcher 暂停游戏节点定格最后帧当底、进面板前才释放；只淡入淡出 GAME OVER/TIME UP 文字，**不显示分数/名字/排名**，播完自动进面板，无按钮）。**面板 = 大标题 YOUR SCORE（1920 座标 (960,80)、96px 居中）＋前 10 名单栏（排名 1ST./名字/分数左对齐 X≈370/620/1255、第一行 Y≈190、行距 72px、60px 字）＋底部当前玩家行（各款主色字＋白边，同格式，名次可能 >10）**，进入时逐行交错显现（淡入＋左滑）；配色：前十行文字白（TEXT）；标题与玩家行各款主色（ACCENT_*）＋白边（8 方向整字偏移外框，1 逻辑 px＝1920 设计 4px）——maze #D80E85／fishing #123BF4／catch #A40CE8；**本局排名 1~3 时玩家行显现起全屏循环播放 `new_record.tscn` 烟花＋循环音效 fireworks_celebration.wav 直到退出**（1600×900 缩到 480×270，音效走 AudioManager 的 play_sfx_loop/stop_sfx_loop，面板 _exit_tree 停；4 名以后不播，缺档跳过）；B/ESC 回该款二级标题并清名字 |
...
| `shared/palette.gd` | 锁定的共用色盘（原 20 色＋2026-09 企划指定的 MOON_LIGHT 与 ACCENT_* 三款排行榜阴影色，共 24 色） |
...
| Catch | ← → | 无（护盾已于 2026-09 取消，B / X 没有功能） | 长按同方向 1 秒线性加速到 ×2.5（A/Shift 冲刺已移除），人物贴屏幕最底 |
...
- **排行榜**：score 降序 → played_at 升序 → record_id 升序；新提交的记录永不因裁剪丢失；记录 ID 由时间戳 + 引擎毫秒 + 同次执行递增序号 + 随机尾码组成（防同毫秒撞号）；档损坏自动改名 `.bak` 重来。**面板 = 大标题 YOUR SCORE＋前 10 名单栏＋底部当前玩家行**，行文字白（TEXT）、标题与玩家行各款主色字＋白边（maze #D80E85／fishing #123BF4／catch #A40CE8），进入时逐行交错显现；**本局排名 1~3 时全屏循环播放 new_record.tscn 烟花＋循环音效直到退出**（玩家行显现时起播）；**Game Over 动画播完自动进面板**（无按钮、无 RESTART，重玩＝回二级重新起名）；B/ESC 回该款二级标题并清名字。
...
- **Catch 露娜已换动画场景（2026-09）**：`assets/AnimationScene/C_Player.tscn`（AnimatedSprite2D 两段：`Idle` 8 帧 @9fps 循环；`Hurt` 3 帧 @6fps，接到炸弹扣命时播一次再回 Idle——炸弹一律扣命，没有挡下分支）。实例挂到子节点 `luna_view`（z=-1、纯视觉、position 只当绘制锚点）；动画不自己播（pause 后 `_process` 手动推帧，launcher 停掉游戏节点时画面才跟着冻）；挤压变形写 `luna_view.scale`（原点＝脚底锚点）；缩放 `PLAYER_SCALE = 0.29`（帧 450×284、内容 308×240 ≈ 判定框 90×71），锚点数据在 `P_REFS`（每段取第 0 帧 alpha>32 包围盒，画师改图要重测）。**背景因此搬到 `BgView` 子节点（z=-2，fishing 同款）**——子节点画在父节点 `_draw()` 之后，背景留在父节点会盖住玩家；场景结构不符时退回 cc_person1 静态贴图。
- **Catch 护盾已取消（2026-09）**：月光能量（cc_05）改为**纯加分道具 +150**、吃 Combo 倍率，仍是每局最多 3 个的稀有物，漏接照样断 Combo；B/X 引爆、8 秒护盾、护盾光圈全部拆除，B/X 键自此无功能。接到炸弹一律扣命。移除后模拟：中位分 4250→4500、存活率 53%→42%、平均局长 53.1→51.3s。
- **Catch 掉落物数量翻倍（2026-09 企划）**：生成间隔减半（0.8~1.1 / 0.7~1.0 / 0.6~0.9 / 0.5~0.75 秒）、同屏上限 2/3/4/5→4/6/8/10；**炸弹比例＝原值 ×0.65**（10%→35% 变 6.5%→22.75%），企划拍板「炸弹增多 30%、不用翻倍」——炸弹一起翻倍会让 AI 存活率 68%→19%、平均局长 56s→45s，局提前结束，总数与分数反而缩水（catch_sim.py 第 6 节）。**末段加压（企划追加）**：第 4 段（45-60s）gap 再 ÷1.3（0.385~0.575）、炸弹比例不动——有价物与炸弹共用生成计时器，两者各 +30%（段内有价物 12.7→16.5 颗、炸弹生成 3.6→4.7 颗）。结果：有价物 28.6→约 59 颗/局、炸弹约 2.6 颗/局、存活率 67%→约 50%、接取率 87%→69%；密度升高稀释了可及性约束的边际价值（第 2 节断言门槛 +0.3→+0.1、接取率 +10→+5）。
...
- **Fishing 钩头是整张贴图**（`assets/fishing/F_Hook.png` 100×100）：绳结点对齐线头、摆动时以绳结点为轴随 `angle` 旋转（Godot 正角顺时针、与 angle 相反所以取 `-angle`）；绳结点贴图坐标在 `HOOK_ROPE`、显示大小 `HOOK_SIZE`，画师改图要重测。钓线颜色 `LINE_COLOR`（#ff81f3），勾中瞬间整条闪白（GDD）。
- **Fishing 左下/右下角固定 4 颗钻石（2026-09）**：一边 2 颗，出生点在**画面角落的 300×500 区域内随机**（1920×1080 设计座标 ÷4 = 逻辑 75×125，`CORNER_DIAMOND_REGION`，锚在屏幕左下/右下角），在随机物件之前放置；是**额外加的，随机 6 颗钻石照旧**。区域大部分在钩子可及锥之外（`LINE_MAX` 205），只有靠内侧一小片捞得到。
- **Fishing 小恶魔之间没有碰撞体积（2026-09）**：互相穿过、可以重叠；旧的 pairwise 推开逻辑会让反向游的两只互相顶住卡死，已移除——不要加回来。
...
- **Seeker 场道具与月光库存已接美术（2026-08-31）**：珍珠 `S_Perl.png`、月光 `S_Moon.png`（均 220×220，`ITEM_SHOW` 等比缩到 15px 居中画在格内，内容物约 11~13px 与旧 15×15 图视觉一致）；HUD 右下月光库存由程序圆形改为 `S_Moon.png`（16px、间距 22；2026-09 从左下角移到右下角并放大），规则同爱心：有库存全亮、空位 alpha 0.22、用掉那颗播 1 秒「放大 1.25 倍＋淡出」（`_moon_fade`/`_moon_fade_slot` 镜像 `_heart_fade`，触发点在 `_activate_moon()` 的 `moon_stock -= 1` 后）。
...
- **`shared/palette.gd` 的 24 色** —— 需要新色先跟企划讨论加进色盘（MOON_LIGHT 与 ACCENT_* 三款阴影色即此流程的先例），不自创中间色。
