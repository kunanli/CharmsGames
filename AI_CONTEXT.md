...
- **Catch 前 30 秒炸弹翻倍（2026-09 企划）**：P1/P2 炸弹比例 ×2（6.5%→13%、13%→26%），P3/P4 不动——开局炸弹太少。与 ×1.2 密度叠加后开局 30 秒约 7.4 颗炸弹（之前 3.7），实测：存活率 32%→**15%**、局长 47.5→**37.9s**、中位分 4500→**2550**、末段有价物 13.3→6.7——**已跌破「炸弹全翻倍 19%」旧悬崖**，多数局活不到第 3/4 段。模拟断言只锁现状不背书（注明确认过悬崖）；太凶先回调 P2 的 26%，其次 P1 的 13%。顾 Combo 优势比压到约 1.06×（门槛 ×1.05）。
