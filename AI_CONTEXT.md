...
**街机按键绑定（2026-09）**：A＝键盘 A／手柄 A、B＝**键盘 S**／手柄 B（键盘 B 的逻辑已全部改到 S，键盘 B 不再有功能）；统一走 InputMap action（`shared/arcade_input.gd`：`arcade_a`／`arcade_b`），同时吃键盘＋手柄的界面用 `_unhandled_input`（手柄按钮事件不进 `_unhandled_key_input`）。方向键未整绑手柄：管理员界面（一级清单／SETTING 二三级）的 ↑↓ 选择与管理员密码的方向输入都额外吃手柄左摇杆（`_pad_stick_nav`／`admin_password.gd` 的 `_stick_direction`，死区 ±0.5、边沿触发不连发、回中立区才能推下一次）；游戏内移动走内建 `ui_left` 等 action（手柄十字键／左摇杆引擎预设已绑）；起名屏本就支持手柄（十字键／左摇杆／A B X Y）。投币系统已于 2026-09 末期整体移除（Y／手柄 View 无特殊功能，开局不扣币，SETTING 的 UNLIMITED COINS 已拆）。
...
- **Catch 再加难：全体 ×1.2＋20 秒起加速（2026-09 企划）**：四段 gap ÷1.2（→0.667~0.917 / 0.583~0.833 / 0.5~0.75 / 0.321~0.479）、同屏上限 ×1.2（→5/7/10/12）、**炸弹比例不动**（跟共享生成计时器一起变密，绝对数量 +20%，「整体 1.2 倍包括炸弹」）；Charm 每 15 秒必出的节拍不变。**加速**：开局 20 秒后落速在段落基准上线性上升（`ACCEL_START` 20 / `ACCEL_RATE` 0.00625），局末累计 +25%；`_fall_speed()` 统一供给 `_move_drops` 实际落速与 `_chain_filter` 可及性估算。模拟（当时）：有价物 59.2→63.9 颗/局、存活率 42%→32%、局长 51.3→47.5s、接取率 70%→65%，中位分持平 4500。
- **Catch 前 30 秒炸弹翻倍（2026-09 企划）**：P1/P2 炸弹比例 ×2（6.5%→13%、13%→26%），P3/P4 不动——开局炸弹太少。与 ×1.2 密度叠加后开局 30 秒约 7.4 颗炸弹（之前 3.7），实测：存活率 32%→**15%**、局长 47.5→**37.9s**、中位分 4500→**2550**、末段有价物 13.3→6.7——**已跌破「炸弹全翻倍 19%」旧悬崖**，多数局活不到第 3/4 段。模拟断言只锁现状不背书；顾 Combo 优势比压到约 1.06×（门槛 ×1.05）。
- **Catch 最新加难：炸弹全局 ×1.3＋10 秒起密度 ×1.5（2026-09 企划）**：四段炸弹比例 →17%/34%/25%/30%，开局 10 秒后掉落物整体 ×1.5（`DENSITY_START` 10 / `DENSITY_MULT` 1.5，gap ÷1.5、max_on ×1.5，有价物与炸弹一起变密，Charm 15 秒节拍不变）；落速加速维持原样（20 秒起 +25%）。10 秒后炸弹流量 ≈ 原设计 4.7 倍（叠加：×1.2 密度 → 前 30 秒炸彈 ×2 → ×1.5 密度 × 炸彈 ×1.3）。实测：开局 30 秒约 13 颗炸弹、存活率 15%→**1%**、局长 37.9→**29.6s**、中位分 2550→**2100**、末段有价物 **2.3**——99% 的局打不满，60 秒局结构与第 3/4 段难度曲线实质不存在；第 2 节设计断言已反转（GDD 直译反超现行 ×1.92 vs ×1.87）。全部断言改为「仅锁现状、不背书」，回调从 ×1.5 密度与炸弹 ×1.3 开始谈。
- **Fishing 四物件改版**（2026-08）：钻石＋200/收线 85/**32×32**（中深层，表面有白光二值闪烁十字星芒）、宝珠＋100/85/32×32（中层）、云朵＋10/160/32×32（任意层）、小恶魔 0 分/105/16×16（任意层，会游动、放线时靠近钩头，上船 −3 秒；**2026-09 起只追以自身为圆心、直径 400px 圆内（1920 座标 ÷4 = `IMP_LURE_RADIUS` 50 逻辑 px）的钩头，不再全图追踪**；**y 轴归位**：每只记住出生 y（`Item.home_y`），没追钩时以 10px/s 游回（1px 死区），追钩不会把全场 imp 拉成一层；**俯仰**：有垂直移动时 ±45° 朝下/朝上姿态（`Item.tilt`，镜像时旋转反向已补偿）；**朝向一律跟实际移动方向走**（`Item.facing`，`_draw` 只读）——旧版「线一出去全体转头看钩子」的 bug 已修）。生成区域（企划直接调过）：(112,168)/(162,208)/(212,256)、全区 (112,256)；**数量翻倍** 4 宝珠＋6 钻石＋8 云朵＋4 恶魔＝22 件，放置顺序「带最窄的先放」（宝珠→钻石→云朵→恶魔）、尝试 500 次（40 次会偶发短缺，模拟 10000 局零失败）。**四物全部回补**（同类同层），重生延迟 5~15 秒随剩余时间由慢到快；没有补充则盘面总值天花板只有 1680 分。
...
- **Fishing 闲置提示 PRESS A（2026-09）**：开局倒数结束后连续 **3 秒无任何操作**（放线/探看/月光都算输入），在露娜左侧（右缘贴船身左侧 8px、基线 y=88、16px GOLD）跳出抖动的 **PRESS A**（高频双正弦 ±1 逻辑 px，画在 HUD 层）。只在钩子待机（SWING）时累计与显示，任何输入重新计数——按 A 放线即消失；回到待机再闲置 3 秒会再出现（持续引导、非一次性）。门槛 `IDLE_HINT_TIME`。
