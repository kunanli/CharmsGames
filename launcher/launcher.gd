...
const COL_TITLE := Color("FFD37A")
const COL_TEXT := Color("F0F4FF")
const COL_DIM := Color("8890C0")
const COL_KEY := Color("A0DCFF")
const COL_WARN := Color("FF9A6A")
const COL_LUNA := Color("EEB4D2")
...
func _draw() -> void:
	...
	_center("CHARMS GAMES", 54, 26, COL_TITLE)
	_center("A PANDORA MINI ARCADE", 74, 10, COL_DIM)
	...
	for i in GAMES.size():
		...
		var name_col := COL_TEXT if built else COL_DIM

		draw_string(font, Vector2(104, y), "[%d]" % (i + 1),
			HORIZONTAL_ALIGNMENT_LEFT, -1, 14, COL_KEY)
	...
	_center("PRESS 1-3 TO PLAY    ESC TO RETURN", 236, 10, COL_DIM)

	if _notice != "":
		_center(_notice, 256, 10, COL_WARN)

	# 角落放個露娜的剪影當招牌，等美術素材進來再換掉
	draw_rect(Rect2(40, 150, 16, 28), COL_LUNA, false, 1.0)
	draw_rect(Rect2(43, 153, 10, 8), COL_LUNA)
