...
- **船與露娜是整張貼圖**（`assets/fishing/F_Luna.png`，300×300，船體與露娜一體，不再分開畫船身與人物；船底龍骨對齊水面線，錨點是 `LUNA_KEEL` 常量）。已預留 **`luna_view`** 接口做動畫切換：未來換成 tscn 動畫場景時把場景實例 `add_child` 掛到 `luna_view`，`_process` 每幀自動對齊 `_luna_anchor() + _juice.world_offset()`（純視覺節點，position 只當繪製錨點、不參與玩法判定），`_draw_boat()` 偵測到 `luna_view` 非 null 就跳過貼圖。
