...
- 22×14 tile 迷宮，外框牆 + 障礙長條。**障礙每局隨機生成**（2026-09，
  `maze.gd` 的 `_generate_blocks()`）：中央 Logo 牆（`LOGO_SIZE` 5×1）固定，
  其餘每局重抽 —— 形狀一律是 **1 行或 1 列的長條**（2~4 格，隨機翻轉方向；
  不生成多行多列的塊，也不會有 1×1 —— tile 素材畫不出孤立方塊）、**一律與
  外框隔 1 格**（最外圈走道永遠淨空，外圈一圈一定鋪得到珍珠）、長條彼此
  （含 Logo 牆）至少隔一格、四角月光／露娜出生格／貓出生格（`Maze.CAT_HOMES`）
  受保護、總面積對齊舊固定圖的 40 格（`GEN_*` 常數）；生成後 BFS 驗證全走道
  連通，64 次失敗退回 `BLOCKS` 固定圖（後備，`seeker_sim.py` 的對照基準）。
...
- **迷宮牆體已改由邏輯地圖自動鋪成 TileMap（2026-09，`maze_tiler.gd`
  ...
  **Seeker 接入方式**：`_ready()` 建圖層後鋪 `maze.walls` **減去外框格**
  ...
  （centered=true），僅保留 Logo 本體，碰撞照舊是牆；
