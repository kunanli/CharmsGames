...
- 四段落速 60→80→100→120 px/s，每 15 秒一跳，炸彈率 6.5%→22.75%（原值的 0.65 倍，2026-09 掉落物翻倍時企劃拍板「炸彈增多 30%、不用翻倍」，見下）。**全局加速（2026-09 企劃）**：開局 **20 秒後**掉落速度在段落基準上線性上升（`ACCEL_START` 20 / `ACCEL_RATE` 0.00625），局末（60s）累計 **+25%**；`_fall_speed()` 統一供給實際落速（`_move_drops`）與生成可及性估算（`_chain_filter`），兩邊不脫節。前 20 秒維持段落基準速度。
